...
# Outcome-stage runner for the exploratory deal-value size
# split frozen by 48o. Estimates pooled and subgroup ATTs, tests their direct
...
if (any(is.na(c(PANEL_DIR, DESIGN_DIR, WEIGHT_ROBUSTNESS_DIR, OUTPUT_DIR)))) {
  stop(paste0("48q requires --panel-dir=, --design-dir=, ",
...
manifest_path <- file.path(DESIGN_DIR, "deal_value_size_design_manifest.csv")
cohorts_path <- file.path(DESIGN_DIR, "deal_value_size_feasible_cohorts.csv")
size_map_path <- file.path(DESIGN_DIR, "deal_value_size_deal_map.csv")
counts_path <- file.path(DESIGN_DIR, "deal_value_size_counts.csv")
gates_path <- file.path(DESIGN_DIR, "deal_value_size_reporting_gates.csv")
weights_path <- file.path(
  DESIGN_DIR, "weights", "deal_value_size_weights.parquet")
...
if (!all(file.exists(required))) {
  stop("Frozen deal-value-size inputs are incomplete: ",
...
rebalanced_cohorts <- sort(as.integer(utils::read.csv(cohorts_path)$cohort))
if (!identical(paste(rebalanced_cohorts, collapse = ";"), manifest$common_cohorts[[1]])) {
...
if (!identical(sort(unique(size_map$size_group)), c("large", "small")) ||
  ...
  stop("Frozen deal-value-size deal map is invalid")
}
cohorts <- sort(intersect(
  unique(size_map$cohort[size_map$size_group == "small"]),
  unique(size_map$cohort[size_map$size_group == "large"])
))
if (!identical(cohorts, c(1994L, 1995L, 1997:2010))) {
  stop("The Cassi--Ornaghi common financial-size cohort set drifted")
}
size_map <- size_map[size_map$cohort %in% cohorts, ]
primary_counts <- do.call(rbind, lapply(c("small", "large"), function(group) {
  z <- size_map[size_map$size_group == group, ]
  data.frame(
    size_group = group,
    acquisitions = nrow(z),
...
subgroup_panel_sql <- sprintf(paste0(
  "(WITH f AS (SELECT p.*, m.size_group, m.n_supported, m.target_value ",
  "FROM %s p INNER JOIN read_csv_auto(%s) m USING(cohort, deal_id) ",
  "WHERE p.cohort IN (%s)), mass AS (SELECT cohort, size_group, ",
  "SUM(weight) FILTER(arm='treated' AND event_time=-1) tm, ",
  "SUM(weight) FILTER(arm='control' AND event_time=-1) cm FROM f ",
  "GROUP BY cohort, size_group) SELECT f.* EXCLUDE(weight), ",
  "CASE WHEN arm='treated' THEN 1.0 ELSE f.weight*mass.tm/mass.cm END::DOUBLE weight ",
  "FROM f JOIN mass USING(cohort, size_group))"),
  base_panel_sql,
  sql_string(normalizePath(size_map_path, winslash = "/", mustWork = TRUE)),
  cohort_sql
)

specs <- list(
  pooled_common_cohorts = pooled_panel_sql,
  deal_value_size_small = sprintf(
    "(SELECT * EXCLUDE(size_group, n_supported, target_value) FROM %s WHERE size_group='small')",
    subgroup_panel_sql),
  deal_value_size_large = sprintf(
    "(SELECT * EXCLUDE(size_group, n_supported, target_value) FROM %s WHERE size_group='large')",
    subgroup_panel_sql)
)

expected_deals <- c(
  pooled_common_cohorts = nrow(size_map),
  deal_value_size_small = sum(size_map$size_group == "small"),
  deal_value_size_large = sum(size_map$size_group == "large")
)
...
subgroup_governing <- headline_all[
  headline_all$summary == "average_annual_t1_to_t5" &
    headline_all$specification %in% c("deal_value_size_small", "deal_value_size_large") &
    headline_all$governing, ]
joint_coefs <- c(
  deal_value_size_small = unname(fit$beta[["tr_small"]]),
  deal_value_size_large = unname(fit$beta[["tr_large"]])
)
standalone <- setNames(
...
full_unit_sql <- sprintf(paste0(
  ...
  "target_value::DOUBLE target_value FROM read_csv_auto(%s) WHERE cohort BETWEEN 1993 AND 2010), ",
  "p AS (SELECT p.*, m.target_value FROM %s p JOIN m USING(cohort, deal_id)), ",
  "u AS (SELECT roster_row_id, cohort::INTEGER cohort, deal_id::INTEGER deal_id, ",
  "codinv::BIGINT codinv, arm, target_value, MAX(weight)::DOUBLE analysis_weight, ",
  "AVG(patent_count) FILTER(event_time BETWEEN 1 AND 5) - ",
...
log_mean <- with(full_unit[full_unit$treated == 1L, ],
                 weighted.mean(log(target_value), analysis_weight))
full_unit$log_size_centered <- log(full_unit$target_value) - log_mean
...
descriptive_log_interaction <- linear_result_frozen(
  ...
  "treated_x_log_target_value", cfg)
...
pair_table <- lmv2_build_pair_table(
  ...
  "deal_value_size_filtered_identity")
influence_table <- lmv2_prepare_influence_table(
  con, pair_table, pooled_panel_sql, cohorts)
DBI::dbWriteTable(con, "tmp_deal_value_size_map", size_map[
...
contribution_event <- DBI::dbGetQuery(con, sprintf(paste0(
  ...
  "FROM %s i JOIN tmp_deal_value_size_map m USING(cohort, deal_id) ",
...
DBI::dbExecute(con, "DROP TABLE tmp_deal_value_size_map")
...
resolved_est <- standalone[paste0("deal_value_size_", treated_mass$size_group)]
resolved_weighted_average <- sum(treated_mass$share * resolved_est)
resolved_nonidentity <- data.frame(
  diagnostic = "frozen_subgroup_weighted_average_minus_pooled_original_weight_att",
  ...
  reason = paste(
    "Relative P5c weights are frozen, but control mass is renormalised within",
    "cohort-by-value cells so the subgroup counterfactuals need not aggregate exactly."
  ),
  stringsAsFactors = FALSE
)

write_csv(certification_all, "deal_value_size_estimation_certification.csv")
write_csv(primary_counts, "deal_value_size_primary_counts.csv")
write_csv(dynamic_all, "deal_value_size_dynamic.csv")
write_csv(pretrend_all, "deal_value_size_pretrend.csv")
write_csv(headline_all, "deal_value_size_headline.csv")
write_csv(coverage_all, "deal_value_size_coverage.csv")
write_csv(aggregation_anchor, "deal_value_size_same16_aggregation_anchor.csv")
write_csv(formal_difference, "deal_value_size_formal_difference.csv")
write_csv(tooth_check, "deal_value_size_joint_model_tooth_check.csv")
write_csv(descriptive_log_interaction, "deal_value_size_descriptive_log_interaction.csv")
write_csv(contribution_event, "deal_value_size_filtered_contributions_event.csv")
write_csv(contribution_post, "deal_value_size_filtered_contributions_post.csv")
write_csv(identity_check, "deal_value_size_filtered_identity_check.csv")
write_csv(resolved_nonidentity, "deal_value_size_resolved_nonidentity.csv")

estimation_manifest <- data.frame(
  version = "lmv2_deal_value_size_split_frozen_weights_v1",
  designation = "post_hoc_exploratory",
  weight_scheme = "frozen_certified_p5c_weights",
  ...
  placement = "main_table_and_appendix",
...
write_csv(estimation_manifest, "deal_value_size_estimation_manifest.csv")
message(sprintf(
  paste0("Deal-value-size estimation complete: small %.4f; large %.4f; ",
         "difference %.4f (p=%.4f, %s governs)"),
  standalone[["deal_value_size_small"]],
  standalone[["deal_value_size_large"]],
