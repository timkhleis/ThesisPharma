...
# supported-treated-inventor acquisition-size split. Post-acquisition outcomes
...
if (any(is.na(c(WEIGHTS_DIR, OUTPUT_DIR)))) {
  stop("48m requires --weights-dir= and --output-dir=")
}
...
AMENDMENT <- file.path(
  BASE, "notes", "local_match_v2_supported_size_split_amendment.md")
if (!file.exists(AMENDMENT)) stop("Frozen size-split amendment is missing")

SIZE_THRESHOLD_EXPECTED <- 16L
MIN_DEALS <- 10L
MIN_TREATED <- 50L
...
threshold <- as.integer(stats::median(size_map$n_supported))
if (!identical(threshold, SIZE_THRESHOLD_EXPECTED)) {
  stop("Supported-size median drifted: expected 16, observed ", threshold)
}
size_map$size_group <- ifelse(size_map$n_supported <= threshold, "small", "large")
...
write_csv(size_correlations, "supported_size_correlations.csv")

size_summary <- data.frame(
  statistic = c(
    "minimum", "q25", "median", "q75", "maximum", "deals_at_median",
    "top_five_supported_inventor_share"),
  value = c(
    min(size_map$n_supported),
    unname(stats::quantile(size_map$n_supported, .25, type = 7)),
    threshold,
    unname(stats::quantile(size_map$n_supported, .75, type = 7)),
    max(size_map$n_supported),
    sum(size_map$n_supported == threshold),
    sum(sort(size_map$n_supported, decreasing = TRUE)[1:5]) /
      sum(size_map$n_supported)
  ), stringsAsFactors = FALSE
)
write_csv(size_summary, "supported_size_threshold.csv")
write_csv(size_map, "supported_size_deal_map.csv")
...
if (anyNA(j)) stop("At least one roster row lacks a supported-size assignment")
...
if (length(floor_cohorts) < 2L) {
  write_csv(cell_census, "supported_size_cell_census.csv")
...
run_solve <- function(x) {
  specification <- paste0("supported_size_", x$size_group[[1]])
...
if (length(solved_cohorts) < 2L) {
  write_csv(cell_census, "supported_size_cell_census.csv")
  write_csv(solve_diagnostics, "supported_size_weight_diagnostics.csv")
...
for (name in names(solves)) {
  ...
  out <- x[c(
    ...
    "original_final_weight", "size_group", "n_supported", balance_vars
  )]
  out$specification <- paste0("supported_size_", out$size_group)
...
write_csv(cell_census, "supported_size_cell_census.csv")
write_csv(solve_diagnostics, "supported_size_weight_diagnostics.csv")
write_csv(balance_out, "supported_size_balance_before_after.csv")
write_csv(data.frame(cohort = solved_cohorts), "supported_size_feasible_cohorts.csv")

weight_path <- file.path(OUTPUT_DIR, "weights", "supported_size_weights.parquet")
write_parquet(con, weights_out, weight_path, "tmp_supported_size_weights")
...
write_csv(count_rows, "supported_size_counts.csv")
...
write_csv(power, "supported_size_preoutcome_power.csv")
...
gates <- data.frame(
  gate = c(
    "at_least_five_common_cohorts",
    "at_least_one_hundred_deals_per_group",
  ...
  threshold = c(5, 100, 500, 1, 0),
  pass = c(
    length(solved_cohorts) >= 5L,
    min(count_rows$acquisitions[count_rows$sample == "common_feasible"]) >= 100L,
    min(count_rows$supported_treated_inventors[
...
write_csv(gates, "supported_size_reporting_gates.csv")

manifest <- data.frame(
  version = "lmv2_supported_size_split_design_v1",
  designation = "post_hoc_exploratory",
  size_definition = "supported_treated_inventors_after_frozen_support",
  threshold = threshold,
  tie_rule = "small_le_16_large_ge_17",
  deals_at_threshold = sum(size_map$n_supported == threshold),
...
write_csv(manifest, "supported_size_design_manifest.csv")

message(sprintf(
  paste0(
    "Supported-size design complete: threshold %d; %d common cohorts, ",
